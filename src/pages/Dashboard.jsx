...
import CreatResumeForm from '../components/CreatResumeForm'
...
const Dashboard = () => {
  ...
  const fetchAllResumes = async () => {
    try {
      ...
      console.error('Error fetchinf resume:', error);
    }
    finally{
      setLoading(false)
    }
  ...
  return (
    <DashboardLayout>
        <div className={styles.container}>
          ...
          {!loading && allResumes.length === 0 && (
            <div className={styles.emptyStateWrapper}>
              ...
              <button className={styles.createButton} onClick={() => setopenCreatModal(true)}>
          ...
          {!loading && allResumes.length > 0 && (
            <div className={styles.grid}>
              <div className={styles.newResumeCard} onClick={()=> setopenCreatModal(true)}>
              ...
              {allResumes.map((resume) => (
                ...
                isNew={moment().diff(moment(resume.createdAt), 'days') <60 }
                />
              ))}
            </div>
          )}
        ...
        {/* CREATE MODAL */}
        <Modal isOpen={openCreatModal} onClose={()=> setopenCreatModal(false)}
          hideHeader maxWidth="max-w-2x1">
            <div className='p-6'>
              <div className={styles.modalHeader}>
                <h3 className={styles.modalTitle}>Create New Resume</h3>

                <button onClick={()=> setopenCreatModal(false)} className={styles.modalCloseButton}>
                  X
                </button>
              </div>
              <CreatResumeForm onSuccess={()=> {
            ...
            <div className='p-4'>
              <div className=' flex flex-col items-center text-center'>
                ...
                <p className={styles.deleteText}>
                  Are you sure you want to delete this resume? This action cann't be  undone
                </p>
              </div>
            </div>
          </Modal>
